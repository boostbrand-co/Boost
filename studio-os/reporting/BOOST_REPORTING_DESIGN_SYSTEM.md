# BOOST Reporting Design System

Status: V1.2 working standard  
Owner: BOOST  
Applies to: recurring client performance reports, monthly marketing reviews, paid-media reports, social reports, and mixed-channel performance decks.

## 1. Purpose

BOOST reports should look like they came from the same studio without forcing every client into the same visual identity.

The system is:

**BOOST skeleton + client skin.**

BOOST owns the grid, hierarchy, typography behavior, chart language, tables, evidence blocks, pacing, analysis structure, cover grammar, and document craft. The client contributes its logo, primary / secondary brand accents when usable, selected imagery, and relevant brand personality.

A set of reports for six different clients should feel related when viewed side by side. Each report should still feel native to the client it represents.

## 2. Visual calibration

Two references set the quality bar for this system:

### Shared Offices Deck
Use as a reference for:

- high information density without crowding
- slides that feel complete rather than empty
- diagrams that replace explanatory paragraphs
- strong metric hierarchy
- tables and frameworks treated as designed objects
- varied slide families within one coherent deck
- deliberate pacing between evidence, systems, people, and conclusions
- editorial composition rather than repeated dashboard cards

Do not copy its visual identity. Preserve its discipline.

### AB Premium reporting
Use as the reference for reporting language and strategic depth:

- metrics followed by interpretation
- clear period read
- content analysis beyond raw rankings
- paid-media results translated into decisions
- actionable recommendations tied to evidence

V1.2 should preserve that depth while making the client-facing layer easier to understand, moving unnecessary technical detail into appendix, improving visual rhythm, and using the approved BOOST report cover consistently.

## 3. Design objective

The report should feel:

- professional
- editorial
- analytical
- visual
- premium
- easy to scan
- substantive
- intentional
- understandable to a smart non-marketer

It should not feel:

- like a generic analytics export
- like a Canva template used unchanged
- like a SaaS dashboard pasted into slides
- empty for the sake of minimalism
- dense because every metric was included
- technical because the platform exposes technical detail
- text-heavy because the analysis was not visualized
- decorative without explanatory value

The goal is **high useful density**.

## 4. Slide economy

Do not make the deck longer than the material requires.

Prefer one well-composed slide that combines related evidence over three sparse slides repeating the same idea.

A slide should use its canvas well, but it does not need to fill every corner.

### Good density

- one clear argument
- one dominant visual hero
- supporting metrics, proof, or annotations
- enough negative space to separate hierarchy

### Bad density

- five equal cards competing for attention
- a headline and one small sentence floating in a mostly empty slide
- six paragraphs reduced to small type
- decorative graphics added to fill unused space

When a slide feels empty, add **useful visual information**, not filler.

For a normal monthly report, target roughly **8–10 core slides plus appendix** when the material supports it. Extended reporting periods can run longer.

## 5. Core brand architecture

### Client signature

Use:

`CLIENT × BOOST`

The client logo or name leads. BOOST co-signs.

The `×` is the most consistent BOOST signature and may use BOOST orange when contrast allows.

### Brand ownership by element

**Client owns:**

- logo
- primary accent
- secondary brand color when useful
- approved photography / imagery
- category-relevant visual context

**BOOST owns:**

- layout system
- type hierarchy
- chart grammar
- tables
- evidence labels
- section structure
- pagination
- source notes
- report logic
- cover grammar
- QA standard

## 6. Working color system

Use a restrained four-color working palette whenever the client's brand supports it:

1. client primary
2. client secondary or approved supporting tone
3. black / near-black
4. white / warm white

Soft gray may appear as a neutral utility surface, not as a fifth expressive brand color.

BOOST orange is a micro-signature outside this working palette.

### Base neutrals

Use:

- white / warm white for analytical slides
- near-black for high-emphasis, visual evidence, or closing slides
- soft gray for secondary surfaces
- thin neutral rules

Avoid cold dashboard gray when a warmer neutral feels more editorial.

### Client accents

Use the client primary and secondary colors intentionally for:

- section labels
- selected chart series
- key numbers
- rules / separators
- small markers
- table highlights
- selected background environments

Do not flood every slide with client color.

### BOOST orange

BOOST orange is a signature, not a second client palette.

Use it sparingly for:

- the `×` in CLIENT × BOOST
- the thin cover rule
- small BOOST metadata
- occasional annotation or action marker
- final co-sign

Do not make every report orange.

### Semantic color

Positive / negative meaning must follow KPI directionality, not mathematical sign.

Use restrained semantic color only when it improves reading.

Examples:

- cost per result -20% = positive
- revenue -20% = negative
- spend +20% = contextual / neutral until interpreted

Avoid red/green confetti across a deck.

## 7. Visual environments and deck rhythm

The deck should use multiple visual environments without rotating them mechanically.

### Environment A — Warm analytical

White or warm-white background, dark type, subtle rules, client color used selectively.

Best for:

- executive summary
- platform performance
- charts
- comparisons
- action plans

### Environment B — Client-color emphasis

Primary or secondary client color used as a controlled full or partial background.

Best for:

- major turning points
- channel / section emphasis
- strategic synthesis
- transitions

Keep text simple and contrast high.

### Environment C — Near-black / visual evidence

Near-black or black background with strong photography, creative thumbnails, or one high-emphasis conclusion.

Best for:

- content evidence
- paid-media highlights
- section pauses
- close

### Environment D — Soft client wash

A light tint or restrained secondary-brand surface.

Best for:

- strategic read
- what we're learning
- synthesis
- controlled variation between analytical slides

### Rhythm rule

Do not let the deck become a run of identical white slides simply because analytical content is easiest to place on white.

As a working rule, avoid more than **three consecutive core slides with the same dominant surface treatment** unless the narrative genuinely benefits from continuity.

Do not alternate colors slide-by-slide by formula. Change environment when the strategic job changes.

Thumbnail view should feel authored.

## 8. Typography

Use the approved active project typeface when the client has a strong brand system and it is practical for the output environment. Otherwise use BOOST's approved presentation type system.

### Hierarchy

**Display / slide argument**  
Large enough to carry the idea. Prefer sentence case for analytical reports unless the client system requires otherwise.

**Section eyebrow**  
Small, tracked, concise.

**Primary metric**  
Large and immediate. Units and context stay nearby.

**Supporting text**  
Readable at laptop size. Never shrink body copy simply to fit more.

**Metadata / source**  
Small but legible. Use for period, source, account, methodology, or caveat.

### Writing treatment

Do not italicize conclusions as a default design device.

Do not turn every insight into an engineered quote.

Use type weight and composition before adding colored boxes.

## 9. Layout system

Use a consistent underlying grid with multiple layout families.

### Layout family A — Executive overview

Use for:

- period overview
- selected KPIs
- strongest movement
- one concise read

Possible structure:

- 3–5 KPI band
- split lower section: positive developments / areas to address
- one short interpretation block

Avoid ten equal KPI cards.

### Layout family B — Metric + chart

Use for:

- platform performance
- trend analysis
- paid-media results

Possible structure:

- one hero metric
- one chart
- supporting secondary metrics
- one explicit read

### Layout family C — Comparison

Use for:

- campaign comparison when decision-relevant
- platform comparison
- content-format comparison
- period comparison

Possible forms:

- table
- paired columns
- slope / delta chart
- ranked bars

### Layout family D — Content evidence

Use real post / Reel / ad thumbnails when available.

Each content item can include:

- thumbnail
- title / hook
- primary metric
- one secondary metric
- short learning

The creative should be visible enough to understand. Do not reduce five posts to filenames in a spreadsheet-style table if the visual itself explains the result.

### Layout family E — Strategic read

Use for:

- what changed
- what we're learning
- what is working
- what needs attention

Prefer evidence blocks with hierarchy over six identical cards.

Client-facing taxonomy:

- **LECTURA BOOST**
- **DECISIÓN**
- **EN OBSERVACIÓN**

Internal reasoning may still use `RESULT → READING → DECISION`.

### Layout family F — Action plan

Use for next moves.

Prioritize 3–5 actions.

Each action should contain:

- action
- short evidence / reason
- measurement or success condition when useful

The core slide should be scannable. Put implementation mechanics in appendix or speaker notes when the client does not need them to approve the direction.

### Layout family G — Visual interstitial

Use sparingly when a client has relevant photography or creative artifacts.

Good jobs:

- open a section
- show the customer or service context
- make a content insight tangible
- pause after dense analysis

Never use unrelated stock photography.

### Layout family H — Appendix / source notes

Use for:

- full content rankings
- detailed campaign / ad-set tables
- attribution caveats
- platform methodology
- raw metric detail
- secondary diagnostics

Protect the clarity of the core report by moving nonessential technical detail here.

## 10. Image handling

Photography and creative evidence must preserve the source asset correctly.

### Non-negotiable fit rules

- Never stretch, squeeze, skew or distort an image to fill a frame.
- Preserve the original aspect ratio.
- Use a deliberate `cover` crop when the image should fill a frame.
- Use `contain` / letterboxing only when showing the full asset is more important than filling the frame.
- Reposition the crop around the actual subject; do not accept an automatic center crop when it cuts the important area.
- Prefer a different asset over a bad crop.
- If no available image fits the intended composition cleanly, use a non-photographic layout instead.

### Quality rules

- Do not upscale a visibly low-resolution image into a hero.
- Do not use a screenshot as a photographic background when the UI / text becomes unreadable or pixelated.
- Do not invent, redraw or alter client logos to make them fit.
- Creative thumbnails may use their native portrait / square ratios inside a designed frame; do not force them into landscape crops when the content becomes unclear.

Image fit is part of QA, not a cosmetic preference.

## 11. Charts

Charts should look authored for the presentation, not exported from a spreadsheet.

### Chart rules

- one question per chart
- direct labels when possible
- limited series
- meaningful scales
- subtle gridlines
- no 3D
- no rainbow palette
- no decorative pies
- no unnecessary legends
- no default Excel formatting

### Preferred chart forms

- line: trends across time
- horizontal bars: ranked content, campaigns, geographies
- vertical bars: period or category comparisons
- stacked bars: paid / organic or content mix when composition matters
- donut: only for a simple 2–4 part composition where the proportion itself is the point
- table: when exact values matter more than shape

Annotate the chart when a specific inflection or result matters.

## 12. KPI blocks

Do not make every KPI block identical by default.

A hero KPI can be large and integrated with a chart or insight.

Supporting KPI blocks should show:

- metric name in plain language
- current value
- comparison period
- meaningful change
- optional short qualifier

Use the client's actual objective to choose hero metrics.

Avoid acronyms in metric labels when the full phrase remains readable.

## 13. Tables

Tables should be compact and presentation-grade.

Use:

- clear header
- light row rules
- direct alignment
- bold first column when useful
- strong hierarchy for the most important numeric column
- restrained highlighting for winners / exceptions

Avoid:

- alternating row fills by default
- heavy grids
- tiny text
- decorative color on every cell

## 14. Insight treatments

BOOST analysis should visually stand apart from raw data without becoming a repeated colored alert box.

Use several treatments depending on importance:

- short annotation beside a chart
- bottom insight band
- margin note
- evidence / reading split
- one highlighted sentence beneath content evidence
- full strategic-read slide for larger conclusions

Do not repeat `LO QUE SUGIERE ESTO` in the same visual format on every slide.

Consistency comes from hierarchy and language, not one repeated container.

## 15. Client-comprehension design

The presentation should visually reinforce plain-language reporting.

Prioritize:

- business result first
- technical label second
- one clear implication

If a technical metric needs explanation, use a small qualifier rather than turning the slide into a glossary.

Examples:

`Costo por conversación` is better than a hero label `CPR`.

`La audiencia vio el contenido unas 3 veces en promedio` is more client-friendly than making `Frecuencia 3.01` the visual hero.

Do not make technical detail look more important than the business meaning simply because it has more columns.

## 16. Paid-media presentation depth

Match the slide depth to the client's paid-media service scope.

### Add-on / amplification accounts

The client-facing core usually needs:

- investment
- primary result
- cost per result
- one concise trend / implication
- one decision if needed

Detailed campaign names, ad-set structure, attribution windows, CTR, CPC, CPM and ad-level delivery belong in appendix or internal notes unless they explain a material issue.

### Core performance accounts

Deeper campaign comparison, creative diagnostics and funnel metrics can remain in the core deck when they support client decisions.

## 17. Canonical report cover

The BOOST monthly-report cover is a **fixed brand grammar**, not a fresh composition generated for every client.

### Core structure

Use the approved split cover:

- **Left panel:** approximately 72–75% of the slide, always black / near-black.
- **Right panel:** approximately 25–28% of the slide, using a dark tone from the client's verified palette.
- **Far-left rule:** thin BOOST-orange vertical line.
- **Top-left:** BOOST B mark / approved BOOST identifier.
- **Primary title block:** client name + report type / period in the lower-middle left area.
- **Metadata row:** Presented to / Prepared by / Period, kept concise.
- **Bottom-left:** report category / document label when useful.
- **Right panel:** month / year or reporting-period treatment, plus restrained category metadata if needed.

The proportions may adjust slightly for unusually long client names, but the cover should still read as the same BOOST template.

### Color behavior

- Left panel stays black / near-black across clients.
- Right panel uses a **dark client color**, not a bright full-saturation field unless the brand requires it and contrast remains premium.
- BOOST orange remains a signature accent, not the client's replacement color.
- White / soft-gray type provides hierarchy.

### Photography on the cover

Photography is optional, not required.

Use a client / brand image only when:

- the asset is high enough resolution,
- it fits the intended frame naturally,
- the crop strengthens the client identity,
- the image can be placed without harming legibility.

If photography is used:

- preserve aspect ratio,
- crop intentionally,
- never stretch it horizontally or vertically,
- never distort the subject to fit the split layout,
- use the strongest available crop or choose another asset.

If no image fits cleanly, use the split black + dark-client-color cover with no photo. A clean non-photographic cover is always better than a distorted image.

### Required content

- client identity
- `× BOOST` co-sign when appropriate to the specific cover treatment
- report type
- reporting period
- prepared-by / client metadata only when useful

### Do not

- invent a different cover every month
- use random photography to make the cover feel designed
- stretch a brand image
- create decorative metadata that nobody uses
- introduce colors outside the approved client + BOOST system
- leave draft states such as `Para revisión` in the final client export

### Cover QA

Before export:

- [ ] left panel follows the approved black BOOST structure
- [ ] right panel uses a verified dark client tone
- [ ] image aspect ratio is preserved
- [ ] no subject / logo is visibly distorted
- [ ] month, year and period are correct
- [ ] no stale metadata remains
- [ ] final cover requires no Canva repair

## 18. Closing

The close can be one of two forms:

### Strategic close
One concise period takeaway + next-period focus.

### Minimal close
Client × BOOST, period, contact / account lead if relevant.

Do not add generic motivational copy.

## 19. Rhythm across the deck

Thumbnail view should have deliberate variation.

A typical report may move through:

- canonical BOOST report cover
- warm analytical overview
- light data slide
- client-color or dark emphasis moment
- platform evidence
- dark / photographic content evidence
- soft strategic synthesis
- light action plan
- quiet appendix / close

Do not alternate backgrounds mechanically. Change visual environment when the content job changes.

## 20. Density target

A good monthly report usually feels **compact but complete**.

There is no fixed slide count.

Use the fewest slides that allow:

- metrics to remain legible
- charts to remain useful
- content thumbnails to remain visible
- analysis to remain readable
- recommendations to remain specific

If the same idea is repeated on two consecutive slides, combine them.

If one slide carries two unrelated decisions, split it.

## 21. Report language and visual hierarchy

Use `performance-reporting` for analysis and `stop-slop-2` for prose.

The visual hierarchy should make this sequence clear:

**Data → Evidence → Interpretation → Action**

The client should never have to guess which sentence is a fact and which is BOOST's read.

## 22. Client-skin configuration

Each client should have a lightweight reporting config containing:

- client name
- logo asset
- primary accent
- secondary brand color / supporting tone
- preferred background bias: light / dark / mixed
- preferred visual environments
- report language
- currency
- platforms
- business objective
- service role by channel
- strategic priorities / active initiatives
- priority KPIs
- reporting depth by channel
- approved photography / image library when available
- cover-specific dark client tone

Do not redesign the report system for every client.

## 23. Acceptance checklist

Before approval:

### Cohesion
- [ ] looks recognizably produced by BOOST
- [ ] still feels native to the client
- [ ] uses one coherent four-color working system
- [ ] BOOST orange stays a signature, not a competing palette
- [ ] canonical cover is used correctly
- [ ] cover does not need manual replacement

### Slide craft
- [ ] canvas is used intentionally
- [ ] no meaningless empty space
- [ ] no cramped text
- [ ] multiple layout families appear
- [ ] multiple visual environments create rhythm
- [ ] no unnecessary run of identical white core slides
- [ ] every slide has a dominant job
- [ ] no repetitive card grid controls the whole deck

### Visual evidence
- [ ] charts are redrawn cleanly
- [ ] content thumbnails are visible when useful
- [ ] tables are readable at laptop size
- [ ] diagrams encode real logic
- [ ] all images preserve aspect ratio
- [ ] no image, screenshot or logo is stretched or distorted

### Analysis and comprehension
- [ ] data, interpretation, and action are visually distinguishable
- [ ] conclusions do not look more certain than the evidence
- [ ] next steps are prioritized
- [ ] a smart non-marketer can understand the core deck
- [ ] technical detail matches the client's service scope
- [ ] appendix protects the core narrative from unnecessary mechanics
- [ ] strategic priorities are not confused with performance winners

### Final output
- [ ] presentation is client-ready on export
- [ ] draft labels are removed
- [ ] PDF and PPTX preserve hierarchy
- [ ] no normal Canva cleanup step is required

## 24. Governing rule

The report should contain more **useful visual explanation** than a platform dashboard and more **strategic judgment** than an automated analytics export, while remaining easy for the client to understand.

If the client could get the same value by opening Meta Business Suite, the report is not finished. If the client needs a marketing lesson to understand the report, it is not finished either.
